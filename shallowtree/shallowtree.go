...
type SlotsNode struct {
	HashNibbleIndex NibbleIndex // Which nibble of the hash to examine to choose between the slots
	Slots           [16]Slot    // What to do when each of 16 possible nibble values are found at HashNibbleIndex
}
...
func (st *ShallowTree) LookupHash(hash []NibbleVal) PiType {
	...
	for {
		...
		nibbleIndexToExamine := node.SlotsNode.HashNibbleIndex
		mostRecentNibbleIndexExamined = nibbleIndexToExamine
		// Let's check we're not being asked to examine one of the prefix bytes
		if nibbleIndexToExamine < st.PrefixNibblesN {
			panic("Nibble index is part of the prefix")
		}
		// Mark byte index as examined
		unusedNibbles.ClearFlagOrPanic(nibbleIndexToExamine)
		examinedByteValue := hash[nibbleIndexToExamine]
...
func (st *ShallowTree) VisitAllNodes(visitor NodeVisitor) {
	...
	for len(stack) > 0 {
		...
		if currentFrame.node.LeafNode == nil {
			// Iterate across the full fixed 16-slot routing block
			for nibbleVal := NibbleVal(0); nibbleVal < NibbleVal(16); nibbleVal++ {
				slot := &currentFrame.node.SlotsNode.Slots[nibbleVal]
...
func (st *ShallowTree) recurseGenerateNode(inputCopy []HashPi,
	...
	// Try partitioning by each of the (up to 128) nibbles in the hashes. Just the ones we haven't used
	const dummy = NibbleIndex(255)
	nibbleIndex := dummy
	maxEntropyFound := float64(0)
	maxEntropyIndex := nibbleIndex
	for nibbleIndex = NibbleIndex(0); nibbleIndex < st.NibblesLength; nibbleIndex++ {
		unused := unusedNibbles.FlagVal(nibbleIndex)
		if unused {
			entropy := partitioningEntropy(inputCopy, nibbleIndex)
	...
	// Use the best one
	bi := maxEntropyIndex
	if bi < st.PrefixNibblesN {
		panic("Byte index is part of the prefix")
	}
	unusedNibbles.ClearFlagOrPanic(bi)
	node.SlotsNode.HashNibbleIndex = bi
	...
	// We have decided to split this node into 16 (fork the tree) based on the value found in the hashes
	// at byte index bi. Consider each value we might find at bi, and what to do.
	index := 0
	for nibbleVal := NibbleVal(0); nibbleVal < 16; nibbleVal++ {
		...
		for index < len(inputCopy) && inputCopy[index].Hash[bi] == nibbleVal {
			index++
		}
		if index == startIndex {
			...
			newNode.NibbleValueLeadingHere = nibbleVal
			newNode.LeafNode = &leafNode
			newNode.SlotsNode = nil
			node.SlotsNode.Slots[nibbleVal].NextNode = &newNode
		} else if index > startIndex+1 {
			// Found more than one, we need a fully fledged slots node child
			childNode := st.recurseGenerateNode(inputCopy[startIndex:index], unusedNibbles, level+1, nibbleVal)
			node.NibbleValueLeadingHere = prevNibbleExamined
			node.SlotsNode.Slots[nibbleVal].NextNode = childNode
		} else {
...
func partitioningEntropy(input []HashPi, hashNibbleIndex NibbleIndex) float64 {
	var nibbleValCounts [16]int
	for i := range input {
		nibbleValCounts[input[i].Hash[hashNibbleIndex]]++
	}
	// Loop over the partitions, calculating probabilities and entropy
	total := float64(len(input))
	entropySum := float64(0)
	for nibbleVal := NibbleVal(0); nibbleVal < 16; nibbleVal++ {
		count := nibbleValCounts[nibbleVal]
